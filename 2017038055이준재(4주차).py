...
while(1) :
    ...
    if task==1 : # 학생 정보 입력
        ...
        if(search in student) == True: # 입력값이 student 함수안에 있을때
            print('학생 정보 \n')
            print(student.get(search)) # 해당 학생 정보 출력
        ...
        deletename=input("이름 : ")
        if(deletename in student) == True: # student안에 입력한 값과 동일한 값이 있으면
            student.pop(deletehakbun) # 해당 학생의 학번을 키로하는 딕셔너리 삭제
            student.pop(deletename) # 해당 학생의 이름을 키로하는 딕셔너리 삭제
